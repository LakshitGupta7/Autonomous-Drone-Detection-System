...
try:
    import serial
    import time
    arduino = serial.Serial('COM6', 9600, timeout=1)  # COM6 System Output
    time.sleep(2)
except Exception as e:
    arduino = None
    print(f"[Arduino Init Error] {e}")

def send_to_arduino():
    if arduino and arduino.is_open:
        try:
            arduino.write(b'1')
            print("[Arduino] Signal sent from process_image")
        except Exception as e:
    ...
    def run(self):
        ...
        while self.running:
            ...
            if not ret or frame is None:
                self.log_signal.emit("❌ Failed to capture frame or frame is None.")
                break

            if np.all(frame == [255, 0, 0]):
                self.log_signal.emit("⚠ Warning: Solid blue frame detected.")


            # ✅ Run detection on the frame
            processed_frame, log_message, latest_frame = process_live_feed(frame, 
                    signal_callback = self.live_signal_plot.update_signal,
                     beep_callback=self.beep_player.play if self.beep_player else None, arduino_callback=send_to_arduino)
